...
Function to estimate dynamics of the vehicle
Model Implemented: Brush Tire Model
Refernece: Hindyenh
*/
...
Vec2d SeDriCa::tire_dynamics(double Ux, double Ux_cmd, double mu, double mu_slide,
...
VecXd SeDriCa::dynamics(const VecXd &x, const VecXd &u)
{
  // std::cout << "x: " << x << '\n';
  // std::cout << "u: " << u << '\n';
  ...
  Vec2d Ff = tire_dynamics(Ux, Ux, mu, mu_spin, G_front, C_x, C_alpha, alpha_F);
  Vec2d Fr = tire_dynamics(Ux, Ux_cmd, mu, mu_spin, G_rear, C_x, C_alpha, alpha_R);
...
VecXd SeDriCa::Integratedk_dynamics(const VecXd &x, const VecXd u){
